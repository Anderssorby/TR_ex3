...
The prediction operator \(P(\cdot|W)\) has the property

\begin{equation}
{\E}[(U-P(U|\bm{W}))^2] = \Var(U) - \bm{a}'\Cov(U,\bm{W}),
\end{equation}

which for the one-step ahead predictor becomes

\begin{equation}
{\E}[(X_{n+1}-P(X_{n+1}|X_n,X_{n-1},\hdots,X_1))^2] = \Var(X_{n+1}) - \bm{a}'\gamma_n,
\end{equation}

where 
\begin{equation}
\gamma_n = (\gamma(1),\gamma(2),\hdots,\gamma(n))',
\end{equation}

\begin{equation}
\Gamma = [\Cov(X_i,X_j)]_{i,j=1}^n
\end{equation}

and \(\bm{a}\) is the solution to \(\Gamma \bm{a} = \gamma\).
```{r prediction}
source('Zestimates.R')
n<- 500
est <- estimateZ(x,n)
