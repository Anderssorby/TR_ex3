...
When the coefficients for \(\psi_i\) are known, the autocovariance function is easily found to be
...
For a causal process, we know that \(X_{t-h}\) and \(\theta(B)Z_t\) are uncorrelated for \(h>q\), hence

\begin{equation}
\gamma(h)=\phi_1\gamma(h-1)+\dots+\phi_p\gamma(h-p).
\end{equation}

which gives \(\gamma(h)\) from \(\gamma(0),\hdots,\gamma(p-1\) (which can be found by solving the set of equations from \eqref{eq:gamma} where \(h<p\)). 
...
This then gives a set of equations containing the autocovariance function  \(\gamma(h)=\E[X_t X_{t+h}]\) and the parameters. By using the sample autocovariance function \(\hat \gamma(h)\) in place of the autocovariance function we can solve the equations, resulting in estimates of the parameters of interest.
...
sigma <- sqrt(sigma2)
```
```{r acvf,fig.cap="The autocovariance $\\hat{\\gamma}(h)$ \\label{fig:acf}"}
lag.max <- 100
...
We can simulate the process and compute the parameters from the simulated process to get an empirical estimation of the distribution of the parameters. 
...
These simulations gives us an estimate of the mean and variance of the parameters.
...
ts.plot(c(x,fore$pred),col=1:2,xlim=c(420,510))
U <- fore$pred+fore$se
L <- fore$pred-fore$se
xx = c(time(U), rev(time(U))); yy = c(L, rev(U))
polygon(xx, yy, border = 8, col = gray(.6, alpha = .2))
...
The autocovariance plot in figure \ref{fig:acf} shows at first glance a tendency to a period. However the value is rather low and might rather be the result of some cyclical noice. 

# Conclusion

The data we were given fits nicely with the assumption that it is an $ARMA(2,1)$ process. Additionally the $Z_t\sim \bf{WN}(0,\sigma^2)$ are likely normaly distributed.
