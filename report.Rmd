...
lines(seq(1,lag.max),ARMAacf(ar=ar,ma=ma,lag.max=lag.max,pacf=TRUE),col=2,lty=3)
```
...
for (i in 1:n_sim) {
  ts_sim <- arima.sim(n=500, list(ar=ar, ma=ma,order=c(2,0,1)),sd=sigma)
  ts <- arima(ts_sim, order = c(2,0,1))
...
which immediately gives the one-step ahead predictor for the \(AR(p)\) process
...
\begin{equation}
\hat X_{n+1} = \sum_{i=1}^{\infty} \pi_{i}X_{n+1-i},
\end{equation}

where the coefficients \(\pi_i\) are given by equation \eqref{eq:picases}.
In figure \ref{fig:prediction} we plot the process \(X_t\) in red along with the one-step ahead predictions \(\hat{X_t}\) in blue. 

```{r prediction,fig.cap="\\label{fig:prediction}"}
source('Zestimates.R')
n<- 500
est <- estimateZ(x,n)
...
plot(x,col=2,type="l",xlim=c(1,500),main="One-step ahead predictions",ylab=TeX("X_t,\\hat{X}_t,Z_t"),xlab="Time")
lines(xhat,col=4)
lines(Z,col=3)
```
<!--
...
plot.arma_prod <- function(prod) {
  ...
  plot(pred[1:l], main = "Model prediction", ylab = TeX("X_t,\\hat{X}_t"), xlab = "t", type = "l", col="blue")
